...
make -j $(nproc)
qemu-system-x86_64 -nographic -m 8 -cpu max -kernel out/c-bye_qemu-x86_64
```
...
UK_DEFCONFIG=/tmp/defconfig make defconfig
make -j $(nproc)
qemu-system-aarch64 -nographic -machine virt -m 8 -cpu max -kernel out/c-bye_qemu-arm64
```
...
The unikernel application image file is `out/c-bye_<plat>-<arch>`, where `<plat>` is the platform name (`qemu`, `fc`, `xen`), and `<arch>` is the architecture (`x86_64` or `arm64`).
...
The current configuration use a single source file: `bye.c`.
...
```make
$(eval $(call addlib,appcbye))

APPCHELLO_SRCS-y += $(APPCHELLO_BASE)/bye.c
